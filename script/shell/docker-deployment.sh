...
mkdir django-volume
sudo docker run -itd -p 8100:8100 --network stock-data-analyze_network-default --ip 172.21.0.40 --volume ${HOME}/django-volume:/shared --name stock-data-analyze-batch stock-data-analyze-batch:latest
