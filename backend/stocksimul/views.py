...
def stock_simul_result(request, pk):
    ...
    if event_info_status is None or (datetime.datetime.now() - event_info_status.mod_dt) \
    ...
    if price_info_status is None or (datetime.datetime.now() - price_info_status.mod_dt) \
            > timedelta(days=PRICE_INFO_UPDATE_PERIOD):
        info_none_yn = price_info_status is None
        if info_none_yn:
            price_info_df = stock.get_market_ohlcv_by_date('2000-01-01', today, event_info.event_code)
        else:
        ...
        price_info_df.rename(columns={'날짜': 'date', '시가': 'open', '종가': 'close', '거래량': 'volume', '고가': 'high', '저가': 'low'},
                       inplace=True)
    ...
    simul_start_date = start_date.strptime('%Y%m%d')
    simul_end_date = end_date.strptime('%Y%m%d')
